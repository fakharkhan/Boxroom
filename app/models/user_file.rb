class UserFile < ActiveRecord::Base
  ...
  has_attached_file :attachment,
                    :path => ':rails_env/:id/:style/:id',
                    :storage => :s3,
                    :s3_permissions => 'private',
                    :s3_protocol => 'https',
                    :bucket => proc { Setting.instance.bucket_name },
                    :s3_credentials => proc {
                      {
                          :access_key_id => Setting.instance.s3_access_key_id,
                          :secret_access_key => Setting.instance.s3_secret_access_key
                      }
                    }
